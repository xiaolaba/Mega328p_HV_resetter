// // by xiaolaba
/*
//default fuses for ATmega328p (Arduino UNO / NANO)
...
void RST_12V_off(){
  digitalWrite(RST, HIGH);  // Turn off 12V, NPN swtich only, invverting
}
void RST_12V_on(){
  digitalWrite(RST, LOW);  // Turn off 12V, NPN swtich only, invverting 
}
// 
*/
...
#define  BAUD         115200  //9600    // Serial port rate at which to talk to PC
...
*/
//default for ATmega328p (Arduino UNO / NANO)
#define  LFUSE        0xFF
#define  HFUSE        0xDE
#define  EFUSE        0xFD
...
#define  XA0     8
#define  XA1     A4    
#define  RST     A0    // 12V supply control pin, NPN swtich used, as an invertor only (12V_EN), mega328p 11.5V
#define  XTAL1   A3
...
void setup() { // run once, when the sketch starts
  ...
  //digitalWrite(VCC, LOW);
  RST_12V_off(); // Turn off 12V
  VCC_off();	 // Turn off VCC

  Serial.begin(BAUD);  // Open serial port, this works on the Mega also because we are using serial port 0
  Serial.println("HVRescueShield, ref https://mightyohm.com/files/hvrescue21/HVRescue_Shield_r21_schematic.png");
  Serial.println("12V supply, NPN switch, inverted RST code design, xiaolaba testing");
  Serial.println("changed to 115200 BAUD\n");
